...
static unsigned int vita_vdp2_cache_frame;

static u32 VitaVdp2Hash(const void *data, unsigned int size)
{
...
static u32 VitaVdp2ParameterHash(const vdp2rotationparameter_struct *source)
{
   vdp2rotationparameter_struct parameter = *source;
   parameter.Xp = parameter.Yp = parameter.dX = parameter.dY = 0.0f;
   parameter.Xsp = parameter.Ysp = parameter.dx = parameter.dy = 0.0f;
   parameter.lkx = parameter.lky = 0.0f;
   parameter.KtablV = 0;
   parameter.charaddr = 0;
   return VitaVdp2Hash(&parameter, sizeof(parameter));
}

static u32 VitaVdp2LineStateHash(int height)
{
   const u16 global_values[] = {
      Vdp2Regs->CCCTL, Vdp2Regs->CCRLB, Vdp2Regs->LCTA.part.U
   };
   u32 hash = VitaVdp2Hash(global_values, sizeof(global_values));
   int line;
   if (height > 270)
      height = 270;
   for (line = 0; line < height; line++) {
      const u16 values[] = {
         Vdp2Lines[line].CLOFEN, Vdp2Lines[line].CLOFSL,
         Vdp2Lines[line].COAR, Vdp2Lines[line].COAG,
         Vdp2Lines[line].COAB, Vdp2Lines[line].COBR,
         Vdp2Lines[line].COBG, Vdp2Lines[line].COBB
      };
      hash ^= VitaVdp2Hash(values, sizeof(values));
      hash *= 16777619U;
   }
   return hash;
}

static void VitaVdp2BuildCacheKey(VitaVdp2CacheKey *key,
                                  ...
                                  int width, int height)
{
   ...
   key->width = width;
   key->height = height;
   key->line_color_base = info->LineColorBase;
   key->info[0] = info->cellw;
   key->info[1] = info->cellh;
   key->info[2] = info->mapwh;
   key->info[3] = info->planew;
   key->info[4] = info->planeh;
   key->info[5] = info->pagewh;
   key->info[6] = info->patternwh;
   key->info[7] = info->patterndatasize;
   key->info[8] = info->specialfunction;
   key->info[9] = info->specialcolorfunction;
   key->info[10] = info->specialcolormode;
   key->info[11] = info->specialcode;
   key->info[12] = info->addr;
   key->info[13] = info->charaddr;
   key->info[14] = info->paladdr;
   key->info[15] = info->colornumber;
   key->info[16] = info->isbitmap;
   key->info[17] = info->supplementdata;
   key->info[18] = info->auxmode;
   key->info[19] = info->alpha;
   key->info[20] = info->coloroffset;
...
static int VitaVdp2CacheKeyEqual(const VitaVdp2CacheKey *a,
                                 const VitaVdp2CacheKey *b)
{
   u32 hash_a = VitaVdp2Hash(a, sizeof(*a));
   u32 hash_b = VitaVdp2Hash(b, sizeof(*b));
   return hash_a == hash_b && memcmp(a, b, sizeof(*a)) == 0;
}

static int VitaVdp2CacheKeyEqualIgnoringDimensions(
...
{
   unsigned int i;
   vita_vdp2_cache_frame++;
   if (!vita_vdp2_cache_frame)
      vita_vdp2_cache_frame = 1;
   for (i = 0; i < VITA_VDP2_CACHE_SLOTS; i++)
      vita_vdp2_cache[i].pinned = 0;
}

static void VitaVdp2CacheReset(void)
{
   memset(vita_vdp2_cache, 0, sizeof(vita_vdp2_cache));
   vita_vdp2_cache_frame = 0;
}

static VitaVdp2CacheEntry *VitaVdp2CacheAcquire(
   const VitaVdp2CacheKey *key, unsigned int kind,
   unsigned int width, unsigned int height, int *hit)
{
   ...
   *hit = 0;
   ...
   for (i = 0; i < VITA_VDP2_CACHE_SLOTS; i++) {
      ...
      if (!candidate->valid || candidate->kind != kind)
         continue;
      same_kind = 1;
      if (VitaVdp2CacheKeyEqual(&candidate->key, key)) {
         if (Vdp2TextureCacheValidateDependencies(
                &candidate->dependencies, &ram_changed, &cram_changed)) {
            candidate->pinned = 1;
      ...
      if (VitaVdp2CacheKeyEqualIgnoringDimensions(&candidate->key, key))
   ...
   if (!entry) {
      for (i = 0; i < VITA_VDP2_CACHE_SLOTS; i++) {
         ...
         if (!candidate->pinned && candidate->last_use < oldest) {
...
static u32 FASTCALL Vdp1ReadPolygonColor(vdp1cmd_struct *cmd)
{
	...
	case 0:
	{
		...
		if (!SPD) color = 0x00;
			...
			if (SPCCCS == 0x03 ){
				u16 checkcol = Vdp2ColorRamReadWord(((colorBank + colorOffset)<<2)&0xFFF );
				if (checkcol & 0x8000){
					u32 talpha = 0xF8 - ((colorcl << 3) & 0xF8);
					talpha |= priority;
					color = Vdp2ColorRamGetColor(colorBank + colorOffset, talpha);
			...
			}else if (temp != 0x0000)
			{
				...
				if (shadow != 0)
				{
					...
					if (((Vdp2Regs->CCCTL >> 6) & 0x01) == 0x01)
					{
						...
						case 3:
						{
							u16 checkcol = Vdp2ColorRamReadWord(((temp + colorOffset) << Vdp2Internal.ColorMode) & 0xFFF);
							if (checkcol & 0x8000){
								alpha = 0xF8 - ((colorcl << 3) & 0xF8);
							}
						}
	...
	case 2:
	{
		...
		if (!SPD) color = 0x00;
			...
			if (SPCCCS == 0x03){
				u16 checkcol = Vdp2ColorRamReadWord(((colorindex) << Vdp2Internal.ColorMode) & 0xFFF);
				if (checkcol & 0x8000){
					u32 talpha = 0xF8 - ((colorcl << 3) & 0xF8);
					talpha |= priority;
...
static u32 Vdp2ColorRamGetColor(u32 colorindex, int alpha)
{
   switch(Vdp2Internal.ColorMode)
   {
      ...
      case 1:
      {
         u32 tmp;
         colorindex <<= 1;
         tmp = Vdp2ColorRamReadWord(colorindex & 0xFFF);
      ...
      case 2:
      {
         ...
         colorindex <<= 2;
         colorindex &= 0xFFF;
         tmp1 = Vdp2ColorRamReadWord(colorindex);
         tmp2 = Vdp2ColorRamReadWord(colorindex+2);
...
{
   u32 tmp;
   tmp = Vdp2ColorRamReadWord((colorindex<<1) & 0xFFF);
   return SAT2YAB1(alpha,tmp);
}
...
void Vdp2GenLineinfo( vdp2draw_struct *info )
{
   ...
   for( i = 0; i < vdp2height; i += info->lineinc )
   {
      ...
      {
		  info->lineinfo[lineindex].LineScrollValH = Vdp2RamReadWord(info->linescrolltbl + (i / info->lineinc)*bound);
      ...
      {
		  info->lineinfo[lineindex].LineScrollValV = Vdp2RamReadWord(info->linescrolltbl + (i / info->lineinc)*bound + index);
      ...
      {
         val1=Vdp2RamReadWord(info->linescrolltbl+(i/info->lineinc)*bound+index);
         val2=Vdp2RamReadWord(info->linescrolltbl+(i/info->lineinc)*bound+index+2);
   ...
   switch(info->colornumber)
   {
      case 0: // 4 BPP
         for(i = 0;i < info->cellh;i++)
         {
            ...
            {
               u16 dot = Vdp2RamReadWord(info->charaddr & 0x7FFFF);
	  ...
	  if (info->isbitmap && info->islinescroll) // Nights Movie
      {
         for(i = 0;i < info->cellh;i++)
         {
            ...
            for(j = 0;j < info->cellw;j++)
            {
               ...
               addr = baseaddr + (j<<1);
               dot = Vdp2RamReadWord(addr & 0x7FFFF);
    ...
    case 4: // 32 BPP
	  if (info->isbitmap && info->islinescroll) // Nights Movie
      {
         for(i = 0;i < info->cellh;i++)
         {
            ...
            for(j = 0;j < info->cellw;j++)
            {
               ...
               u32 addr;
   			   if( Vdp2CheckWindowDot(info,j,i)==0 ){ *texture->textdata++=0; continue; }
	   		   addr = baseaddr + (j<<2);
               dot1 = Vdp2RamReadWord(addr & 0x7FFFF);
               dot2 = Vdp2RamReadWord((addr+2) & 0x7FFFF);
         ...
         for(i = 0;i < info->cellh;i++)
         {
            ...
            {
               u16 dot1, dot2;
               dot1 = Vdp2RamReadWord(info->charaddr & 0x7FFFF);
               info->charaddr += 2;
               dot2 = Vdp2RamReadWord(info->charaddr & 0x7FFFF);
               info->charaddr += 2;
               if (!(dot1 & 0x8000) && info->transparencyenable) color = 0x00000000;
               else color = SAT2YAB2(info->alpha, dot1, dot2);
               *texture->textdata++ = info->PostPixelFetchCalc(info, color);
...
static void Vdp2PatternAddr(vdp2draw_struct *info)
{
   switch(info->patterndatasize)
   {
      case 1:
      {
         u16 tmp = Vdp2RamReadWord(info->addr);
      ...
      case 2: {
         u16 tmp1 = Vdp2RamReadWord(info->addr);
         u16 tmp2 = Vdp2RamReadWord(info->addr+2);
         info->addr += 4;
...
static void Vdp2PatternAddrPos(vdp2draw_struct *info, int planex, int x, int planey, int y )
{
	...
	case 2: {
		u16 tmp1 = Vdp2RamReadWord(addr);
		u16 tmp2 = Vdp2RamReadWord(addr+2);
		info->charaddr = tmp2 & 0x7FFF;
		info->flipfunction = (tmp1 & 0xC000) >> 14;
		switch (info->colornumber) {
		case 0:
...
static void Vdp2DrawMapPerLine(vdp2draw_struct *info, YglTexture *texture){
	...
	for (v = 0; v < info->drawh; v += info->lineinc){  // ToDo: info->coordincy
		...
		if (info->isverticalscroll)	{
			...
			targetv += Vdp2RamReadLong(info->verticalscrolltbl) >> 16;
		}
...
static INLINE u32 Vdp2RotationFetchPixel(vdp2draw_struct *info, int x, int y, int cellw)
{
   ...
   {
      case 0: // 4 BPP
         dot = Vdp2RamReadByte(((info->charaddr + ((y * cellw) + x) / 2) & 0x7FFFF));
         if (!(x & 0x1)) dot >>= 4;
      ...
      case 1: // 8 BPP
         dot = Vdp2RamReadByte(((info->charaddr + (y * cellw) + x) & 0x7FFFF));
      ...
      case 2: // 16 BPP(palette)
         dot = Vdp2RamReadWord(((info->charaddr + ((y * cellw) + x) * 2) & 0x7FFFF));
      ...
      case 4: // 32 BPP
         dot = Vdp2RamReadLong(((info->charaddr + ((y * cellw) + x) * 4) & 0x7FFFF));
...
static void FASTCALL Vdp2DrawRotation(vdp2draw_struct *info, vdp2rotationparameter_struct *dmy, YglTexture *texture)
{
   ...
   Vdp2 * regs;
#ifdef VITA_TEXTURE_CACHE
   VitaVdp2CacheKey cache_key;
   VitaVdp2CacheEntry *main_cache = NULL;
   VitaVdp2CacheEntry *line_cache = NULL;
   int main_cache_hit = 0;
   int line_cache_hit = 0;
   int cache_ready = 0;
#endif
   ...
   if (info->LineColorBase != 0)
      ...
      line_cache_hit = 1;
   cache_ready = main_cache != NULL &&
                 (info->LineColorBase == 0 || line_cache != NULL);
   if (!cache_ready) {
      if (main_cache && main_cache->valid == 2) {
         main_cache->valid = 0;
         main_cache->pinned = 0;
      }
      if (line_cache && line_cache->valid == 2) {
         line_cache->valid = 0;
         line_cache->pinned = 0;
      }
   }
   if (cache_ready && main_cache_hit && line_cache_hit) {
      YglCache cached;
      if (info->LineColorBase != 0) {
   ...
   if (info->LineColorBase != 0 && line_cache)
      VitaProfileRecordVdp2PersistentCache(0, 0);
   if (cache_ready)
   ...
   if( info->LineColorBase !=0 )
   {
      ...
      VitaProfileSetVdp2Source(VITA_PROFILE_VDP2_SOURCE_ROTATION_LINE);
	   ...
	   LineColorRamAdress = (Vdp2RamReadWord(info->LineColorBase) & 0x7FF);// +info->coloroffset;
   }else{
   ...
   {   
      ...
      {
		  LineColorRamAdress = (Vdp2RamReadWord(info->LineColorBase) & 0x7FF);
...
//	  if (regs) ReadVdp2ColorOffset(regs, info, info->linecheck_mask);
   ...
   if (cache_ready) {
      ...
      if (line_cache)
         line_cache->dependencies = main_cache->dependencies;
      main_cache->valid = 1;
      if (line_cache)
         line_cache->valid = 1;
   }
...
void VIDOGLVdp1DrawStart(void)
{
   ...
   u8 *sprprilist = (u8 *)&Vdp2Regs->PRISA;
#ifdef VITA
   VitaProfilePushAtlasPhase(VITA_PROFILE_ATLAS_VDP1);
#endif
#ifdef VITA_PROFILE
...
static void Vdp2DrawLineColorScreen(void)
{
  ...
  for (i = 0; i < line_cnt; i++){
    ...
    u16 LineColorRamAdress = Vdp2RamReadWord(addr);
...
vdp2rotationparameter_struct * FASTCALL vdp2rGetKValue2W( vdp2rotationparameter_struct * param, int index )
{
   ...
   kdata = Vdp2RamReadLong((param->coeftbladdr&0x7FFFF) + (index<<2) );
...
vdp2rotationparameter_struct * FASTCALL vdp2rGetKValue1W( vdp2rotationparameter_struct * param, int index )
{
   ...
   kdata = Vdp2RamReadWord(param->coeftbladdr + (index<<1) );
