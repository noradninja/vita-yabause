...
static void progress(const char *step) {
  ...
  psvDebugScreenPrintf("\033[1;1HAri64 VM validation - revision 2\033[K\n\n"
    "Cycle %d / 3     Rewrite %d / 100\033[K\n"
...
static void finish_screen(const char *detail) {
  SceCtrlData pad;
  if (!screen_ready) return;
  psvDebugScreenPrintf("\033[2J\033[1;1HAri64 VM validation - revision 2\n\n%s\n\n"
...
int main(void) {
  ...
  fprintf(logfile, "test=ari64-vm revision=2 build=%s_%s run=%llu bytes=%u sh2_execution=NOT_TESTED\n",
  ...
  for (lifecycle = 0; lifecycle < 3 && !failure; ++lifecycle) {
    ...
    iteration_number = 0;
    progress("Allocating 16 MiB of executable memory...");
    ...
    progress("Releasing executable memory...");
  ...
  finish_screen(failure ? "See the last reported step in the log." : "All three memory lifecycles passed.");
